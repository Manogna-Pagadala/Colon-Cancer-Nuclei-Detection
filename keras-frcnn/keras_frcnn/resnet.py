...
def nn_base(input_tensor=None, trainable=False):
   ...
   # x = FixedBatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    ...
    print('shape after conv1 maxpool',x.shape)
    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='d', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='e', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='f', trainable = trainable)
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='g', trainable = trainable)

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='e', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='f', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='g', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='h', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='i', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='j', trainable = trainable)
